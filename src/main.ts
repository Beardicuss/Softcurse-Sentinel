...
Sentry.init({
  ...
  release: `softcurse-sentinel@${__APP_VERSION__}`,
  environment: location.hostname.includes('localhost') || location.hostname.includes('127.0.0.1') ? 'development'
...
// In web production, route RPC calls through the configured API edge.
